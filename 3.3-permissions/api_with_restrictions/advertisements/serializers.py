...
class AdvertisementSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get('status'
                    ) == 'OPEN' or self.context['request'].method == 'POST':
...
class UserFavoriteAdvertisementSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        request_adv = Advertisement.objects.filter(
            id=self.context['request'].data['advertisement'])
        ...
        if UserFavoriteAdvertisement.objects.filter(
                ...
                advertisement=request_adv[0]).count() > 0:
            raise ValidationError('Уже добавлено в избранное')
        if request_user == request_adv[0].creator:
            raise ValidationError(
                'Пользователь не может добавть свое объявление в избранное')

        return OrderedDict([('like_user', request_user),
                            ('advertisement', request_adv[0])])
