...
def test_list_courses(client, student_factory, course_factory):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data[0]['id'] == course_list[0].id and data[0]['students'] == [
        student.id for student in students_list_1
    ]
    assert data[1]['id'] == course_list[1].id and data[1]['students'] == [
        student.id for student in students_list_2
    ]
...
def test_post_course(client, student_factory):
    ...
    response1 = client.post(path='/api/v1/courses/',
                            data={
                                ...
                                'Курс Нетология',
                                'students':
                                [student.id for student in students_list]
                            },
                            format='json')
